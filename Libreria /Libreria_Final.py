...
def json_save():
    with open("./bookshop.json", "w", encoding = "utf8") as file: #  "w" means write, encoding is the accents, 
        json.dump({"books": bookshop}, file, ensure_ascii=False, indent=4) 

def write_data_in_json(what, where): #Alternative from function line 7, explained. This is more correct as the above is only for bookshop
    with open(where, "w", encoding="utf8") as file:
        json.dump(what, file, ensure_ascii=False, indent=4)


# write_data_in_json(bookshop, "./bookshop.json") # here it would be what we would use if we wanted to write
...
def create_book(genre, bookshop):
    new_book = {}
    keys = list(bookshop[0].keys()) # que es ['id', 'title', 'author', 'genre']
...
while user != "q":
    ...
    elif user == "7": # TERCER ALTERNATIVA CON BOOK_BY_ID + I (indice)
        user = input("id: ")
